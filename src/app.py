...
st.title("Data Analyst (MCP + SQLite)")
st.markdown("Ask questions about your **local transaction data**. No data leaves this container.")

# Sidebar for Settings
with st.sidebar:
    st.header("Settings")
    model_id = st.text_input("OpenAI Model", DEFAULT_MODEL)
    api_key_input = st.text_input("OpenAI API Key", type="password", value=OPENAI_API_KEY)
    if api_key_input:
...
if prompt := st.chat_input("Ex: How many failed payments in Germany yesterday?"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            # Send context + user prompt to OpenAI
            ...
            st.warning("Make sure you provided a valid API key!")